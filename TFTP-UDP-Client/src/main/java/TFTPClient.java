...
public class TFTPClient implements IClient {
	...
	public boolean getFile(String filename) throws TFTPException {
		...
		DatagramPacket datagramPacket = new DatagramPacket(buffer, TFTPRequestBuilder.MAX_BYTES, host, port);
		try {
		...
		// Unpack the opcode
		TFTPRequestBuilder.OPCODE opcode = TFTPRequestDecoder.unpackOp(data);

		if (opcode == TFTPRequestBuilder.OPCODE.ERROR) {
			// Unpack the error code and message.
			TFTPRequestDecoder.ErrorPacket errorPacket = null;
			try {
				errorPacket = TFTPRequestDecoder.unpackError(data, 0);
			} catch (TFTPException e) {
		...
		try {
			packet = TFTPRequestDecoder.unpackData(datagramPacket.getData(), 0);
		} catch (TFTPException e) {
			return false;
		}
